...
int main()
{
    ...
    std::vector<uint8_t> p(128+128*(10+1));
	unsigned char plaintext[16];
	unsigned char expandedkey[16*(10+1)];
	for(int i = 0; i < 16; i++) plaintext[i] = U8dist(engine);
	for(int i = 0; i < 16*(10+1); i++) expandedkey[i] = U8dist(engine);
    
    for (int i = 0; i < 16; i++) for(int j = 0; j < 8; j++) p[i*8+j] = (plaintext[i] >> j) & 1;
    for (int i = 0; i < 16*(10+1); i++) for(int j = 0; j < 8; j++) p[128+i*8+j] = (expandedkey[i] >> j) & 1;
